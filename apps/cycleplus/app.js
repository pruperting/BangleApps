/*
 * =============================================================
 * Cycle Plus - v7.3 (Layout Improvement)
 * =============================================================
 * A GPS cycling computer with ghost race and commute tracking.
 *
 * - UI: Redesigned the main data layout for improved clarity.
 * The Distance-based (D) and Location-based (L) ghost metrics
 * have been moved to the bottom of the screen to de-clutter
...
function draw() {
  ...
  g.setColor(g.theme.fg).setFont("6x8", 2).setFontAlign(0, 1); // Center Align
  g.drawString(durationStr, r.w / 2, bottomY);

  // Ghost comparisons (Left and Right of Duration)
  if (ghostTrack.length > 0 && isRunning) {
    // Distance-based diff (Left)
    let distDiffStr = "D " + (distTimeDiff > 0 ? "+" : "") + Math.round(distTimeDiff) + "s";
    g.setColor(distTimeDiff > 0 ? "#f00" : "#0f0");
    g.setFontAlign(-1, 1); // Left Align
    g.drawString(distDiffStr, r.x + 4, bottomY);

    // Location-based diff (Right)
    let locDiffStr = "L " + (locTimeDiff > 0 ? "+" : "") + Math.round(locTimeDiff) + "s";
    g.setColor(locTimeDiff > 0 ? "#f00" : "#0f0");
    g.setFontAlign(1, 1); // Right Align
    g.drawString(locDiffStr, r.w - 4, bottomY);
  }
